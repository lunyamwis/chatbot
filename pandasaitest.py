...
def refer_from_the_knowledgebase_tool(message):
    ...
    * Return only values found in the dataframe period do not hallucinate.
...
def vehicle_enquiry_agent(user_message=None) -> str:
    ...
    If `{user_message}` indicates that the user is sharing their budget, only treat it as provided when they explicitly state a numeric value greater than 0. Trigger the `negotiate_car_price` tool to figure out if a lowest price can be found.
    ...
    if tool_results:
        for result in tool_results:
            if result.function.name == "negotiate_car_price":
                ...
                return {"reply": f"{tool_result['deal_text']}"}
